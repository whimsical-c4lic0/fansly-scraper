...
class TestNormalizeFilenameIntegration:
    ...
    async def test_normalize_filename_with_extensions(self, entity_store, config, ext):
        """Test normalize_filename with a database match across extensions.

        The jpg row is the former test_normalize_filename_with_database, whose
        body was this exact Account+Media setup, rename, and idempotence walk.
        """
        store = entity_store
